...
interface ComputeInstanceProvisionedResourcesTabProps {
...
const ComputeInstanceProvisionedResourcesTab = ({
  catalogItemId,
}: ComputeInstanceProvisionedResourcesTabProps) => {
  ...
  const { data, isLoading, error } = useComputeInstancesForCatalogItem(catalogItemId, {
    limit: perPage,
    offset,
  });
  ...
  return (
    <ProvisionedResourcesTable
      rows={(data?.items ?? []).map(computeInstanceRow)}
      total={data?.total ?? 0}
      isLoading={isLoading}
      error={error}
      page={page}
...
export default ComputeInstanceProvisionedResourcesTab;
