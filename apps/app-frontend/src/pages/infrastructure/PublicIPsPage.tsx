  ...
  return (
    <PageSection isFilled>
      ...
      {!isLoading && !error && (
        <OcTable
          ...
          columns={[
            ...
            { label: 'Attached To', render: (pip) => pip.spec.computeInstance ? vmName(pip.spec.computeInstance) : '—' },
            { screenReaderText: 'Actions', isActionCell: true, render: (pip) => <ActionsColumn items={rowActions(pip)} /> },
          ]}
        />
      )}
      ...
      <Modal isOpen={allocateModal} onClose={() => setAllocateModal(false)} variant="small" aria-labelledby="allocate-pip-modal-title">
        ...
        <ModalBody>
          {submitError && <Alert variant="danger" title="Allocate failed" isInline className={modalAlertCss}>{submitError}</Alert>}
          <Form>
      ...
      {attachModal && (
        <Modal isOpen onClose={() => setAttachModal(null)} variant="small" aria-labelledby="attach-pip-modal-title">
          ...
          <ModalBody>
            {submitError && <Alert variant="danger" title="Attach failed" isInline className={modalAlertCss}>{submitError}</Alert>}
            <Form>
