...
export function upsertComputeInstanceInCache(qc: QueryClient, updated: ComputeInstance): void {
...
export function removeComputeInstanceFromCache(qc: QueryClient, id: string): void {
  qc.setQueriesData<PageOfT<ComputeInstance>>({ queryKey: ['compute_instances'] }, (old) => {
    if (!old?.items) return old
...
export function appendComputeInstanceToCache(qc: QueryClient, created: ComputeInstance): void {
  qc.setQueriesData<PageOfT<ComputeInstance>>({ queryKey: ['compute_instances'] }, (old) => {
    if (!old?.items) return old
