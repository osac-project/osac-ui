...
export const isPendingVmClientId = (id: string): boolean => {
  return id.startsWith(PENDING_VM_CLIENT_ID_PREFIX);
};

export const createPendingVmClientId = (): string => {
  return `${PENDING_VM_CLIENT_ID_PREFIX}${crypto.randomUUID()}`;
};

export const matchesPendingCreation = (
  pending: PendingVmCreation,
  vm: ComputeInstance,
): boolean => {
  if (pending.serverId && vm.id === pending.serverId) {
    return true;
  }
...
export const pendingToComputeInstance = (pending: PendingVmCreation): ComputeInstance => {
...
export const resolveCreationDisplayState = (
  startedAtMs: number,
  nowMs = Date.now(),
): VmPowerState => {
...
export const createPostCreateWatch = (vmId: string, startedAtMs: number): PostCreateWatch => {
  return {
    vmId,
    startedAtMs,
    seenStarting: false,
...
const postCreateInProgressDisplay = (watch: PostCreateWatch, nowMs: number): VmPowerState => {
 ...
 */
export const advancePostCreateWatch = (
